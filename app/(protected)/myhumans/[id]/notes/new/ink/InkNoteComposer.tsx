...
const TldrawNoteCanvas = dynamic(
  ...
  { ssr: false, loading: () => <div className="w-full h-full bg-card animate-pulse" /> },
)
...
export default function InkNoteComposer({ subjectPersonId, subjectName }: Props) {
  ...
  return (
    <div className="flex flex-col h-[100dvh] bg-card">
      ...
      {/* ── Top bar ─────────────────────────────────────────────────────── */}
      <header className="flex items-center gap-2 px-3 py-2 bg-card border-b border-border flex-shrink-0">
        <button
        ...
        <div className="min-w-0">
          <p className="text-xs uppercase tracking-wide text-muted-foreground leading-none">Ink note for</p>
          <p className="text-sm font-semibold text-foreground truncate leading-tight">{subjectName}</p>
        </div>
      ...
      {/* ── Caption + status ─────────────────────────────────────────────── */}
      <footer className="px-3 pb-3 pt-2 flex-shrink-0 space-y-2 bg-card border-t border-border">
        <input
          value={caption}
          onChange={(e) => setCaption(e.target.value)}
          placeholder="Optional caption…"
          className="w-full rounded-md border border-border bg-card px-3 py-2 text-sm placeholder:text-muted-foreground focus:outline-none focus:ring-1 focus:ring-[hsl(213,70%,30%)]"
          disabled={saving}
        />
